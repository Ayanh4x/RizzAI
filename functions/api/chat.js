 ...
 * Proxies NVIDIA NIM server-side so browser CORS is bypassed.
 */
...
export async function onRequestPost(context) {
  try {
    ...
    if (!nimRes.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `AI error ${nimRes.status}`, detail: errText.slice(0, 300) }),
    ...
    return new Response(JSON.stringify({ reply, emo }), { status: 200, headers: CORS });
